...
cp "$esp/efi-files.zip" "$out/efi-files.zip"
zstd -T2 -6 --no-progress --stdout "$esp/esp.img" > "$out/esp.img.zst"
if [[ "$image" == *.zst ]]; then
...
Keep enough disk space for the decompressed images. Flash esp.img to esp;
follow docs/installation.md for the rootfs and storage variant instructions.
EOF
(
  cd "$out"
  sha256sum esp.img.zst efi-files.zip "$raw_name.zst.part-"* \
    SHA256SUMS.images RESTORE.txt > SHA256SUMS
)
